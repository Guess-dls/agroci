...
export const ContactRequestsList = () => {
  ...
  const loadRequests = useCallback(async () => {
    ...
    try {
      ...
      const validRequests = (data || [])
        .map((request) => ({
          ...request,
          buyer_profile: Array.isArray(request.buyer_profile)
            ? request.buyer_profile[0]
            : (request.buyer_profile || null),
          product: Array.isArray(request.product) ? request.product[0] : request.product,
        }))
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`producer-requests-${profileId}`)
      .on(
        "postgres_changes",
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Acceptez les demandes pour discuter via la messagerie intégrée</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {requests.length === 0 ? (
          <p className="text-muted-foreground text-center py-4">Aucune demande de contact en attente</p>
        ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant="outline">{new Date(request.created_at).toLocaleDateString("fr-FR")}</Badge>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button onClick={() => handleAccept(request.id)} disabled={processingId === request.id} size="sm">
                        <Check className="h-4 w-4 mr-1" />
                        Accepter
                      </Button>
                      <Button onClick={() => handleReject(request.id)} disabled={processingId === request.id} variant="outline" size="sm">
